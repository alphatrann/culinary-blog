...
| Thành phần     | Yêu cầu                                                                                                                                                                                                                                                                                        |
| -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Python + uv    | Python 3.12+, `uv` làm package/dependency manager (pyproject.toml + uv.lock). API chạy trực tiếp trên host (`uv run`, hot reload) — không containerize API/nginx ở development, xem ADR-0009                                                                                                   |
| Node.js        | Node.js 20+ LTS với npm 10+                                                                                                                                                                                                                                                                    |
| Docker Desktop | Docker Desktop 4.x+ (Windows/macOS) hoặc Docker Engine (Linux) – chỉ chạy dependency: PostgreSQL, 3×Redis, MinIO (`compose.development.yml`), mỗi service publish port ra `localhost`. Không chạy nginx/API container, không chạy Grafana stack; structured JSON log đọc trực tiếp qua console |
...
- PostgreSQL, 3×Redis và MinIO được cung cấp qua Docker Compose trong development và dưới dạng managed service (hoặc VPS) trong production.
...
2. Chuẩn hóa cả `q` và `title` qua `f_unaccent(lower(...))`.
3. Query: `WHERE f_unaccent(lower(title)) %> f_unaccent(lower(:q))` (word_similarity, dùng GIN index).
4. `ORDER BY word_similarity(:q, title) DESC`.
5. Chỉ trả `status == published`.
6. Áp dụng filter/pagination, trả `PagedResult` kèm `relevance_score` (`word_similarity`, 0..1). |
   ...
   | HTTP Method & Endpoint | `GET /api/v1/recipes/search?q={term}&page={n}&page_size={n}&category_id={id}&difficulty={level}&max_cook_time={min}` |
   | Kết quả mong đợi | `PagedResult<RecipeSummaryOut>` kèm `relevance_score`, xếp hạng theo `word_similarity`. Hỗ trợ tìm kiếm không dấu tiếng Việt. |
...
Module cung cấp khả năng quan sát (Observability) theo ba trụ cột: Logging, Metrics, Distributed Tracing. Logging có cấu trúc (JSON, `correlation_id`) là bắt buộc theo CONS-010 ở **mọi** môi trường. Ở production, cả ba trụ cột xuất qua OpenTelemetry tới bộ Grafana stack (Tempo/Loki/Prometheus/Grafana, xem Chương 6); ở development, structured log chỉ xuất ra console (`docker compose logs`) — không chạy OTel Collector/Tempo/Loki/Prometheus/Grafana. Ở mức FR chỉ Health Check được đặc tả như một yêu cầu chức năng riêng biệt; logging/tracing được mô tả dưới dạng ghi chú triển khai đi kèm.
...
**Ghi chú triển khai — Structured Logging:** Mọi HTTP request được log dạng JSON với `correlation_id` (header `X-Correlation-ID`), method/path/status, thời gian xử lý (ms), `user_id` (khi đã xác thực). Middleware `CorrelationIdMiddleware` gán/forward `correlation_id`. Cảnh báo khi request > 500ms. Log xuất ra stdout ở mọi môi trường; production thu thập qua OpenTelemetry Collector → Loki, development đọc trực tiếp qua console.

**Ghi chú triển khai — Distributed Tracing & Metrics:** OpenTelemetry Python SDK instrument cho: HTTP request traces, database query traces (SQLModel/asyncpg), custom business metrics (recipe created/published count). Ở production, traces/metrics xuất qua OTLP tới Tempo/Prometheus và xem trên Grafana Dashboard; `trace_id` được include trong structured log để log-trace correlation. Ở development, pipeline OTLP/Tempo/Prometheus/Grafana không chạy — `trace_id` vẫn xuất hiện trong console log nhưng không có dashboard để xem.
...
Toàn bộ hệ thống containerized với Docker Compose, khớp `container-diagram.png` — nhưng chỉ ở production. `compose.development.yml` chỉ chạy các dependency (Postgres, 3×Redis, MinIO) với port publish ra `localhost`; API chạy trực tiếp trên host qua `uv run` để có hot reload thật. `compose.production.yml` chạy toàn bộ, gồm nginx và API containerized. Cấu hình per-service (Redis conf, nginx conf, observability conf) được build vào image riêng qua `docker/*.Dockerfile` thay vì bind-mount, để tránh phụ thuộc vào Docker Desktop file-sharing settings.

| Service                   | Image / Build                                  | Port (host:container)     | Volume / Dependency                                                                                                                                                                                                                                                                                                              |
| ------------------------- | ---------------------------------------------- | ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| nginx                     | `docker/nginx.Dockerfile`                      | 80:80, 443:443            | **Production only**. Depends: api, web                                                                                                                                                                                                                                                                                           |
| web                       | `culinaryblog-web` (Dockerfile)                | 3000:3000                 | Depends: api                                                                                                                                                                                                                                                                                                                     |
| api                       | `Dockerfile`                                   | 8000:8000                 | **Production only** — development chạy API trực tiếp qua `uv run` (`uv run alembic upgrade head` rồi `uv run culinary-blog`), kết nối dependency qua `localhost:<port>`. Prod depends: migrate (`service_completed_successfully`), cache-redis, queue-redis, ratelimit-redis, minio. Env: `.env.development` / `.env.production` |
| migrate                   | `Dockerfile` (`command: alembic upgrade head`) | —                         | **Production only**. One-shot, `restart: "no"`. Depends: postgres. Runs schema migrations once per deploy — not embedded in `api`'s entrypoint, so scaling `api` to N replicas never races concurrent `alembic upgrade` runs                                                                                                     |
| postgres                  | `postgres:16-alpine`                           | 5432:5432                 | Volume: `pgdata:/var/lib/postgresql/data`                                                                                                                                                                                                                                                                                        |
| cache-redis               | `docker/redis-cache.Dockerfile`                | 6380:6379                 | `maxmemory 2gb`, `maxmemory-policy allkeys-lfu` — không persistence bắt buộc (xem ADR-0003)                                                                                                                                                                                                                                      |
| queue-redis               | `docker/redis-job-queue.Dockerfile`            | 6381:6379                 | Volume: `queuedata:/data`. `appendonly yes`, `appendfsync everysec`, `maxmemory-policy noeviction` (xem ADR-0004)                                                                                                                                                                                                                |
| ratelimit-redis           | `docker/redis-rate-limit.Dockerfile`           | 6382:6379                 | Volume: `ratelimitdata:/data`. `maxmemory 4gb`, `maxmemory-policy allkeys-lru`, RDB `save 60 1` (xem ADR-0007)                                                                                                                                                                                                                   |
| minio                     | `quay.io/minio/minio:latest`                   | 9000:9000, 9001:9001      | Volume: `miniodata:/data`                                                                                                                                                                                                                                                                                                        |
...
| otel-collector            | `docker/otel-collector.Dockerfile`             | 4317:4317 (OTLP gRPC)     | **Production only**. Depends: tempo, loki, prometheus                                                                                                                                                                                                                                                                            |
| tempo / loki / prometheus | `docker/{tempo,loki,prometheus}.Dockerfile`    | internal                  | **Production only**. Volume riêng cho từng service                                                                                                                                                                                                                                                                               |
| grafana                   | `docker/grafana.Dockerfile`                    | 3001:3000                 | **Production only**. Depends: tempo, loki, prometheus                                                                                                                                                                                                                                                                            |
